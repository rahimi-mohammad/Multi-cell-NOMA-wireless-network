...
% ------------------------------------------------------------------------
function [W , rate , Z_cvx] = Rate3(h_d, G, h_r, N1, N, P_T, noise_power ,...
                                epsilon)
    M_t = size(h_d, 2) ;
...
%     load('W.mat')
    ...
    if N == 0
        ...
        elseif M_t>1
            ...
            [h_d, ~, ~] = CCSOrder(h_d, G, h_r) ;
            [rate, W] = MISONoma2(epsilon, h_d , noise_power , P_T) ;
        ...
        elseif M_t>1
            ...
            h = zeros( N1 , M_t) ; % Cascade channel
            rate0 = 0 ;
            rate2 = 0.2 ;
            feasible = false ;
            while feasible == false
                disp('regenerating W') 
                W = 1 * randn(M_t , N1)/sqrt(4 * N1 * M_t/ P_T) ; 
                [~, Z_cvx , feasible] = MISONoma1(W , epsilon, h_d , G , h_r , noise_power) ;
            end
