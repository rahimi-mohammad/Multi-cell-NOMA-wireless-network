...
for N1 = start : step : (SV - 1) * step + start
    ...
    for k = 1 : N_iter
        ...
        final_rate(floor((N1 - start)/ step) + 1) = final_rate(floor((N1 - start)/ step) + 1) +  exact_rate ;
        rate_lower_bound = log2(1 + P_T * channel_gain(1)^2/noise_power)/N1 ;
        final_Lband(floor((N1 - start)/ step) + 1) = final_Lband(floor((N1 - start)/ step) + 1) +  rate_lower_bound ;
        %% random theta
        rand_theta = exp(1j * 2 * pi * rand(N,1))  ;
        channel_gain = abs(h_d + h_r' * diag(rand_theta) * g)  ;
        channel_gain = sort(abs(channel_gain))  ;
        rate_lower_bound = log2(1 + P_T * channel_gain(1)^2/noise_power)/N1 ;
        random_final_rate(floor((N1 - start)/ step) + 1) = random_final_rate(floor((N1 - start)/ step) + 1) + rate_lower_bound ;
