...
tic
% parameters
N1 = 6 ;                                 % No. first Basestations users
N2 = 6 ;                                 % No. first Basestations users
R1 = 15 ;                                % BS1 user area center radius
...
% draw BS1 users
plot(x(1),y(1),'o-','MarkerSize',90 * R1/10,'linewidth',0.1)
% plot(x(1),d,'o-','MarkerSize',90 * R1/10,'linewidth',0.1)
% draw BS2 users
plot(x(N1+1),start,'o-','MarkerSize',90 * R1/10,'linewidth',0.1)
% configs
title('User Location')
xlabel('x')
ylabel('y')
...
% draw results
set(gca,'FontName','Times New Roman','FontSize',11,'LineWidth',1.5) ; 
plot(start : step : (SV-1) * step+start, EPG_final_rate(1,1 : i),'b-*','linewidth',1)
hold on
% plot(start : step : (SV-1) * step+start,Random_final_rate(1,1 : i),'b','linewidth',1)
plot(start : step : (SV-1) * step+start,EPG_final_rate(2,1 : i),'r-*','linewidth',1)
% plot(start : step : (SV-1) * step+start,Random_final_rate(2,1 : i),'r','linewidth',1)

plot(start : step : (SV-1) * step+start, NE_final_rate(1,1 : i),'b-o','linewidth',1)
plot(start : step : (SV-1) * step+start,NE_final_rate(2,1 : i),'r-o','linewidth',1)
...
nexttile
% draw results
plot(start : step : (SV-1) * step+start , EPG_final_utility(1 , 1 : i) , 'b-*' , 'linewidth' , 1)
hold on
% plot(start : step : (SV-1) * step+start , Random_final_utility(1 , 1 : i) , 'b' , 'linewidth' , 1)
plot(start : step : (SV-1) * step+start , EPG_final_utility(2 , 1 : i) , 'r-*' ,  'linewidth' , 1)
% plot(start : step : (SV-1) * step+start , Random_final_utility(2 , 1 : i) , 'r' , 'linewidth' , 1)


plot(start : step : (SV-1) * step+start ,  NE_final_utility(1 , 1 : i) , 'b-o' , 'linewidth' , 1)
plot(start : step : (SV-1) * step+start , NE_final_utility(2 , 1 : i) , 'r-o' , 'linewidth' , 1)
legend('BS1 EPG' , 'BS2 EPG' ,  'BS1 NE' ,  'BS2 NE') ; 
% legend('BS1 EPG' , 'BS1 RANDOM'...
%      , 'BS2 EPG' , 'BS2 RANDOM') ; 
% configs
